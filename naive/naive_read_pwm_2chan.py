...
def read_pwm_pins():
    ...
    while True:
        ...
        # Throttle Next
        if (throttle_value == True) & (last_state_throttle == False):
            # Pin went from low to high; Start timing
            throttle_start = ticks_us()
            last_state_throttle = True
        else:
            if (throttle_value == False) & (last_state_throttle == True):
                # Pin went from low to high; Stop timing
                throttle_stop = ticks_us()
                throttle_us = ticks_diff(throttle_start, throttle_stop)
                last_state_throttle = False
                print("Throttle: ",throttle_us)
